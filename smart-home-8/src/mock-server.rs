...
fn main() {
    ...
    let temperature_levels = Arc::new(vec![21.0, 22.0, 23.0, 24.0, 25.0, 26.0, 27.0, 28.0, 29.0]);
    let power_level_counter = Arc::new(Mutex::new(0));
    let temperature_level_counter = Arc::new(Mutex::new(0));

    let udp_threads: Vec<thread::JoinHandle<thread::Result<()>>> = udp_addresses
        .into_iter()
            ...
            thread::spawn(move || -> thread::Result<()> {
                ...
                loop {
                    ...
                    if u8::from_le_bytes(buf) == SEND_TEMPERATURE {
                        ...
                        socket
                            .send_to(&f64::to_le_bytes(temperature_level), client_address)
                            .unwrap();
                    }
                }
            })
